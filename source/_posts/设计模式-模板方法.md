...
![类结构](/images/image-20250102202215030.png)
...
对于`BaseSolutionServiceTest`可扩展的点在于`setUp`方法，父类会抽象出一些公共的初始化，每个子类都要用到，复用如下
...
我为什么要这样做呢？因为之前发布，合并代码的时候，有个人对`AbstractSolutionService`的`buildBusinessSolutionToolCard`方法新增了一个参数，导致所有继承自`AbstractSolutionServiceTest`的类对应的单侧类都要同步修改，从而引发的很多冲突。
